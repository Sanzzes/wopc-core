...
char remotes[NUM_REMOTES][MAX_REMOTE] = {
    "git@ixi-soft.com:ixiliumemu.git",
    "git://ixi-soft.com/ixiliumemu.git"        // used for fetch if present
};
...
bool check_fwd()
{
    ...
    if (!found)
    {
        ...
        {
            printf("ERROR: non-fastforward, use rebase!\n");
            return false;
        }
    }
    return true;
...
int get_rev(const char *from_msg)
{
    ...
    if (sscanf(from_msg, "[%[0123456789]]", nr_str) != 1)
        return 0;

    if (from_msg[strlen(nr_str)+1] != ']')
        return 0;
...
bool find_rev()
{
    ...
    for (int i = 0; i < NUM_REMOTES; i++)
    {
        if (!local && !origins[i][0])
            continue;

        if (local)
            snprintf(cmd, MAX_CMD, "git log HEAD --pretty=\"format:%%s\"");
        else
...
std::string generateNrHeader(char const* rev_str, char const* date_str, char const* time_str, char const* wow_str)
{
    ...
    if (!strcmp(rev_str,"") || !strcmp(rev_str,"*"))
    {
    ...
    newData << " #define STRFILEVER     \"0, 0, " << rev_str << "\""<< std::endl;
    newData << " #define STRPRODUCTVER  \"0, 0, " << rev_str << "\""<< std::endl;
...
void system_switch_index(const char *cmd)
{
    ...
    if (!use_new_index)
        return;
    if (putenv(new_index_cmd) != 0)
        return;

    system(cmd);

    if (putenv(old_index_cmd) != 0)
        return;
}
...
bool write_rev_nr()
{
    ...
    std::string header = generateNrHeader(rev_str, date_str, time_str, wow_str);
...
bool amend_commit()
{
    ...
    if (use_new_index && putenv(new_index_cmd) != 0)
        return false;
    ...
    if (use_new_index && putenv(old_index_cmd) != 0)
        return false;
...
bool get_sql_update_info(const char *buffer, sql_update_info &info)
{
    ...
    if (sscanf(buffer, "%d_%d_%[^_]_%[^.].sql", &info.rev, &info.nr, info.db, info.table) != 4 && sscanf(buffer, "%d_%d_%[^.].sql", &info.rev, &info.nr, info.db) != 3)
    {
        info.rev = 0;       // this may be set by the first scans, even if they fail

        if (sscanf(buffer, "%d_%[^_]_%[^.].sql", &info.nr, info.db, info.table) != 3 && sscanf(buffer, "%d_%[^.].sql", &info.nr, info.db) != 2)
            return false;
    }

    for (info.db_idx = 0; info.db_idx < NUM_DATABASES; info.db_idx++)
        if (strncmp(info.db, databases[info.db_idx], MAX_DB) == 0)
            break;
...
bool find_sql_updates()
{
    ...
    while (fgets(buffer, MAX_BUF, cmd_pipe))
    {
        ...
        {
            if (info.rev > 0 && (info.rev > last_sql_rev[info.db_idx] || (info.rev == last_sql_rev[info.db_idx] && info.nr > last_sql_nr[info.db_idx])))
            {
...
bool convert_sql_updates()
{
    ...
    for (std::set<std::string>::iterator itr = new_sql_updates.begin(); itr != new_sql_updates.end(); ++itr)
    {
        ...
        if (!get_sql_update_info(itr->c_str(), info))
            return false;
...
bool change_sql_database()
{
    ...
    for (int i = 0; i < NUM_DATABASES; i++)
    {
        ...
        while (1)
        {
            ...
            if (sscanf(buffer, "  `required_%s`", dummy) == 1)
                break;
...
bool change_sql_history()
{
    ...
    if (hashes.empty())
        return false;    // must have at least one commit

    if (hashes.size() < 2)
        return true;  // only one commit, ok but nothing to do
...
bool prepare_new_index()
{
    ...
    if (old_index)
        strncpy(src_file, old_index, MAX_PATH);
    else
        snprintf(src_file, MAX_PATH, "%s.git/index", path_prefix);
