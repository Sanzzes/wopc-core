...
LogonList& sLogonList
{
    static LogonList realmlist;
    return realmlist;
}
...
void LogonList::UpdateRealm( uint32 ID, const std::string& name, const std::string& address, uint32 port, uint8 icon, uint8 color, uint8 timezone, AccountTypes allowedSecurityLevel, float popu, uint32 build)
{
...
void LogonList::UpdateRealms(bool init)
{
    sLog.outDetail("Updating LogonList...");

    QueryResult *result = loginDatabase.Query( "SELECT id, name, address, port, icon, color, timezone, allowedSecurityLevel, population, gamebuild FROM logonlist WHERE color <> 3 ORDER BY name" );
