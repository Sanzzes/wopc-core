...
void AuthSocket::_SetVSFields(const std::string& rI)
{
    ...
    v_hex = v.AsHexStr();
    s_hex = s.AsHexStr();
    loginDatabase.PExecute("UPDATE account_settings SET v = '%s', s = '%s' WHERE username = '%s'", v_hex, s_hex, _safelogin.c_str() );
...
bool AuthSocket::_HandleLogonProof()
{
    ...
    if (!memcmp(M.AsByteArray(), lp.M1, 20))
    {
        ...
        loginDatabase.PExecute("UPDATE account_settings SET sessionkey = '%s', last_ip = '%s', last_login = NOW(), locale = '%u', failed_logins = 0 WHERE username = '%s'", K_hex, GetRemoteAddress().c_str(), GetLocaleByName(_localizationName), _safelogin.c_str() );
        ...
        if (_build == 8606 || _build == 9947 || _build == 10505 || _build == 11159)//2.4.3 and 3.1.3 and 3.2.0a clients
        {
    ...
    else
    {
        ...
        if (MaxWrongPassCount > 0)
        {
            ...
            loginDatabase.PExecute("UPDATE account_settings SET failed_logins = failed_logins + 1 WHERE username = '%s'",_safelogin.c_str());

            if (QueryResult *loginfail = loginDatabase.PQuery("SELECT id, failed_logins FROM account_settings WHERE username = '%s'", _safelogin.c_str()))
            {
...
bool AuthSocket::_HandleReconnectChallenge()
{
    ...
    QueryResult *result = loginDatabase.PQuery ("SELECT sessionkey FROM account_settings WHERE username = '%s'", _safelogin.c_str ());
...
bool AuthSocket::_HandleRealmList()
{
    ...
    QueryResult *result = loginDatabase.PQuery("SELECT id,sha_pass_hash FROM account_settings WHERE username = '%s'",_safelogin.c_str());
    if (!result)
    {
    ...
    sLogonList.UpdateIfNeed();

    LogonList::RealmMap::const_iterator rlm;
    LogonList built_realmList;
    for (rlm = sLogonList.begin(); rlm != sLogonList.end(); ++rlm)
    {
        if (_build == 8606 || _build == 9947 || _build == 10505 || _build == 11159)//2.4.3 and 3.1.3 and 3.2.0a clients
        {
    ...
    ByteBuffer pkt;
    pkt << (uint32) 0;
    if (_build == 8606 || _build == 9947 || _build == 10505 || _build == 11159)//2.4.3 and 3.1.3 and 3.2.2a clients
        pkt << (uint16) built_realmList.size();
    else
        pkt << (uint32) built_realmList.size();
    LogonList::RealmMap::const_iterator i;
    for ( i = built_realmList.begin(); i != built_realmList.end(); ++i )
    {
        ...
        if (i->second.gamebuild == 11159 || i->second.gamebuild == 10505 || i->second.gamebuild == 9947 || i->second.gamebuild == 8606)//2.4.3 and 3.1.3 and 3.2.2a clients
    ...
    if (_build == 11159 || _build == 10505 || _build == 9947 || _build == 8606)//2.4.3 and 3.1.3 and 3.2.2a clients
    {
        pkt << (uint8) 0x10;
        pkt << (uint8) 0x00;
