...
int Master::Run()
{
    ...
    // set server online
    loginDatabase.PExecute("UPDATE logonlist SET color = 0, population = 0 WHERE id = '%d'",realmID);
...
//        if (Prio && (m_ServiceStatus == -1)/* need set to default process priority class in service mode*/)
    ...
    loginDatabase.PExecute("UPDATE logonlist SET color = 2 WHERE id = '%d'",realmID);
...
bool Master::_StartDB()
{
    ...
    if (!WorldDatabase.CheckRequiredField("version", REVISION_DB_WORLD))
    {
    ...
    WorldDatabase.PExecute("UPDATE version SET core_revision = '%s'", _FULLVERSION);

    sWorld.LoadDBVersion();

    sLog.outString("Using World DB: %s", sWorld.GetDBVersion());
    sLog.outString("Using GIXI Version: %s", sWorld.GetGIXIVersion());
...
void Master::clearOnlineAccounts()
{
    ...
    loginDatabase.PExecute("UPDATE account_settings SET active_logon_id = 0 WHERE active_logon_id = '%d'", realmID);
