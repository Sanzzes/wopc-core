...
/// Add an broadcast
bool ChatHandler::HandleBroadcastAddCommand(const char* args)
{
    if (!*args)
        return false;

    char *bcText = (char*)args;

    if (!bcText)
        return false;

    // normalized in bcmgr.AddBroadcast
    std::string broadcast_text = bcText;
    BroadcastOpResult result = bcmgr.AddBroadcast(broadcast_text);

    switch(result)
    {
        case BOR_OK:
            PSendSysMessage(LANG_BROADCAST_CREATED, broadcast_text.c_str());
            
            break;
        case BOR_TEXT_TOO_LONG:
            SendSysMessage(LANG_BROADCAST_TOO_LONG);
            SetSentErrorMessage(true);
            return false;
        case BOR_TEXT_ALREADY_EXIST:
            SendSysMessage(LANG_BROADCAST_ALREADY_EXIST);
            SetSentErrorMessage(true);
            return false;
        case BOR_DB_INTERNAL_ERROR:
            PSendSysMessage(LANG_BROADCAST_NOT_CREATED_SQL_ERROR, broadcast_text.c_str());
            SetSentErrorMessage(true);
            return false;
        default:
            PSendSysMessage(LANG_BROADCAST_NOT_CREATED, broadcast_text.c_str());
            SetSentErrorMessage(true);
            return false;
    }
...
/// Edit an broadcast
bool ChatHandler::HandleBroadcastEditCommand(const char* args)
{
    if (!*args)
        return false;
...
/// Delete an broadcast
bool ChatHandler::HandleBroadcastDeleteCommand(const char* args)
{
    if (!*args)
        return false;
