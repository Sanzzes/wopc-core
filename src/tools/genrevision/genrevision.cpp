...
void extractDataFromGit(FILE* EntriesFile, std::string path, bool url, RawData& data)
{
    ...
    if (url)
    {
        ...
        // parse URL like git://github.com/Sanzzes/wopc-core.git
        char url_buf[200];
...
std::string generateHeader(char const* rev_str, char const* date_str, char const* time_str, char const* wow_str)
{
    ...
    {
        newData << " #define FILEVER        1,6," << wow_str << ",0"<< std::endl;
        newData << " #define PRODUCTVER     1,6," << wow_str << ",0"<< std::endl;
    }
    newData << " #define STRFILEVER     \"0, 0, " << wow_str << "\""<< std::endl;
    newData << " #define STRPRODUCTVER  \"0, 0, " << wow_str << "\""<< std::endl;
...
int main(int argc, char **argv)
{
...

    {
        ...
        if (res)
            newData = generateHeader(data.rev_str, data.date_str, data.time_str, data.wow_str);
        else
