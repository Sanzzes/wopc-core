...
void npc_escortAI::EnterEvadeMode()
{
    ...
    if (HasEscortState(STATE_ESCORT_ESCORTING))
    {
        ...
        ReturnToLastPoint();
        debug_log("ISCR: EscortAI has left combat and is now returning to last point");
    }
    else
	{
...
void npc_escortAI::UpdateAI(const uint32 uiDiff)
{
    ...
    if (HasEscortState(STATE_ESCORT_ESCORTING) && !m_creature->getVictim() && m_uiWPWaitTimer && !HasEscortState(STATE_ESCORT_RETURNING))
    {
        if (m_uiWPWaitTimer <= uiDiff)
        {
            ...
            if (CurrentWP == WaypointList.end())
            {
                if (DespawnAtEnd)
                {
                    debug_log("ISCR: EscortAI reached end of waypoints");
                    ...
                    if (m_bCanReturnToStart)
                    {
                        ...
                        debug_log("ISCR: EscortAI are returning home to spawn location: %u, %f, %f, %f", POINT_HOME, fRetX, fRetY, fRetZ);
                        return;
                    }
                ...
                {
                    debug_log("ISCR: EscortAI reached end of waypoints with Despawn off");
            ...
            if (!HasEscortState(STATE_ESCORT_PAUSED))
            {
                ...
                debug_log("ISCR: EscortAI start waypoint %u (%f, %f, %f).", CurrentWP->id, CurrentWP->x, CurrentWP->y, CurrentWP->z);
    ...
    if (HasEscortState(STATE_ESCORT_ESCORTING) && m_uiPlayerGUID && !m_creature->getVictim() && !HasEscortState(STATE_ESCORT_RETURNING))
    {
        if (m_uiPlayerCheckTimer < uiDiff)
        {
            ...
            {
                debug_log("ISCR: EscortAI failed because player/group was to far away or not found");
...
void npc_escortAI::MovementInform(uint32 uiMoveType, uint32 uiPointId)
{
    ...
    {
        debug_log("ISCR: EscortAI has returned to original position before combat");
    ...
    {
        debug_log("ISCR: EscortAI has returned to original home location and will continue from beginning of waypoint list.");
    ...
    else
    {
        ...
        {
            error_log("ISCR ERROR: EscortAI reached waypoint out of order %u, expected %u", uiPointId, CurrentWP->id);
            return;
        }

        debug_log("ISCR: EscortAI Waypoint %u reached", CurrentWP->id);
...
void npc_escortAI::SetRun(bool bRun)
{
    if (bRun)
    {
        if (!m_bIsRunning)
            ...
            debug_log("ISCR: EscortAI attempt to set run mode, but is already running.");
    }
    else
    {
        if (m_bIsRunning)
            ...
            debug_log("ISCR: EscortAI attempt to set walk mode, but is already walking.");
    }
    m_bIsRunning = bRun;
}
