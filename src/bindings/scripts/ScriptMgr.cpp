/* Copyright (C) 2006 - 2008 Scripts <https://scriptdev2.svn.sourceforge.net/>
...
void LoadDatabase()
{
    std::string strISDBinfo = IScriptConfig.GetStringDefault("WorldDatabaseInfo", "");

    if (strISDBinfo.empty())
    {
        error_log("ISCR: Missing world database info from configuration file. Load database aborted.");
        return;
    }

    //Initialize connection to DB
    if (IScriptDB.Initialize(strISDBinfo.c_str()))
    {
        outstring_log("ISCR: Scripts database at %s initialized.", strISDBinfo.c_str());
        outstring_log("");
    ...
    {
        error_log("ISCR: Unable to connect to database at %s. Load database aborted.", strISDBinfo.c_str());
        return;
    }

    IScriptDB.HaltDelayThread();
}
...
void ScriptsInit(char const* cfg_file = "world.conf")
{
    ...
    outstring_log("Scripts initializing %s", _FULLVERSION);

    //Get configuration file
    if (!IScriptConfig.SetSource(cfg_file))
        error_log("ISCR: Unable to open configuration file. Database will be unaccessible. Configuration values will use default.");
    else
        outstring_log("ISCR: Using configuration file %s", cfg_file);
    ...
    outstring_log("ISCR: Loading C++ scripts");
    barGoLink bar(1);
    bar.step();
    outstring_log("");
...
std::string GetConfigValueStr(char const* option)
{
    ...
    std::string dbstring = IScriptConfig.GetStringDefault(option, "");

    if (dbstring.empty())
    {
        error_log("ISCR: %s is not a valid option.", option);
        return "error";
    }
    return dbstring;
}
...
int32 GetConfigValueInt32(char const* option)
{
    ...
    int32 dbint = IScriptConfig.GetIntDefault(option, 0);
...
{
    //Get db int from file
    float dbfloat = IScriptConfig.GetFloatDefault(option, 0);
...
void DoScriptText(int32 iTextEntry, WorldObject* pSource, Unit* pTarget)
{
    if (!pSource)
    {
        error_log("ISCR: DoScriptText entry %i, invalid Source pointer.", iTextEntry);
        return;
    }

    if (iTextEntry >= 0)
    {
        error_log("ISCR: DoScriptText with source entry %u (TypeId=%u, guid=%u) attempts to process text entry %i, but text entry must be negative.", pSource->GetEntry(), pSource->GetTypeId(), pSource->GetGUIDLow(), iTextEntry);
        return;
    }
    ...
    if (!pData)
    {
        error_log("ISCR: DoScriptText with source entry %u (TypeId=%u, guid=%u) could not find text entry %i.", pSource->GetEntry(), pSource->GetTypeId(), pSource->GetGUIDLow(), iTextEntry);
        return;
    }

    debug_log("ISCR: DoScriptText: text entry=%i, Sound=%u, Type=%u, Language=%u, Emote=%u", iTextEntry, pData->uiSoundId, pData->uiType, pData->uiLanguage, pData->uiEmote);
    ...
    if (pData->uiSoundId)
    {
        if (GetSoundEntriesStore()->LookupEntry(pData->uiSoundId))
        {
            ...
            error_log("ISCR: DoScriptText entry %i tried to process invalid sound id %u.", iTextEntry, pData->uiSoundId);
    }
    ...
    if (pData->uiEmote)
    {
        if (pSource->GetTypeId() == TYPEID_UNIT || pSource->GetTypeId() == TYPEID_PLAYER)
            ...
            error_log("ISCR: DoScriptText entry %i tried to process emote for invalid TypeId (%u).", iTextEntry, pSource->GetTypeId());
    }
        ...
        case CHAT_TYPE_WHISPER:
            {
                if (pTarget && pTarget->GetTypeId() == TYPEID_PLAYER)
                    ...
                    error_log("ISCR: DoScriptText entry %i cannot whisper without target unit (TYPEID_PLAYER).", iTextEntry);
            }
            break;
...
void Script::RegisterSelf()
{
    ...
    {
        error_db_log("CRASH ALERT! Scripts: RegisterSelf, but script named %s does not have ScriptName assigned in database.", (this)->Name.c_str());
        delete this;
    }
}
