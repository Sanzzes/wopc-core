...
bool GOHello_go_inconspicuous_mine_car(Player* pPlayer, GameObject* pGO)
{
    ...
    {
        // Hack Why Ixilium Dont Support Custom Summon Location
        if(Creature *miner = pPlayer->SummonCreature(28841, 2383.869629, -5900.312500, 107.996086, pPlayer->GetOrientation(),TEMPSUMMON_DEAD_DESPAWN, 1))
        {
            ...
            if(Creature *car = pPlayer->GetVehicleCreatureBase())
            {
                ...
                } else error_log("ISCR: GOHello_go_inconspicuous_mine_car vehicle entry is not correct.");
            } else error_log("ISCR: GOHello_go_inconspicuous_mine_car player is not on the vehicle.");
        } else error_log("ISCR: GOHello_go_inconspicuous_mine_car Scarlet Miner cant be found by script.");
    }
    return true;
}
