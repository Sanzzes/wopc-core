...
struct SCRIPTS_DLL_DECL npcs_andorhal_towerAI : public ScriptedAI
{
    npcs_andorhal_towerAI(Creature *c) : ScriptedAI(c) {}
    ...
    void MoveInLineOfSight(Unit *who)
    {
		...
		if (who->GetTypeId() == TYPEID_PLAYER)
			if( ((Player*)who)->GetQuestStatus(5097) == QUEST_STATUS_INCOMPLETE && m_creature->FindNearestGameObject(176093,10.0f))
			    ((Player*)who)->KilledMonsterCredit(m_creature->GetEntry(),m_creature->GetGUID());
	}
};

CreatureAI* GetAI_npcs_andorhal_tower(Creature* pCreature)
{
    return new npcs_andorhal_towerAI (pCreature);
}
...
void AddSC_western_plaguelands()
{
    ...
    newscript = new Script;
    newscript->Name = "npcs_andorhal_tower";
    newscript->GetAI = &GetAI_npcs_andorhal_tower;
    newscript->RegisterSelf();
}
