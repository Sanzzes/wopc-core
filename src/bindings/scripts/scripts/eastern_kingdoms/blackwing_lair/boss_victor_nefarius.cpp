...
struct SCRIPTS_DLL_DECL boss_victor_nefariusAI : public ScriptedAI
{
    ...
    void BeginEvent(Player *pTarget)
    {
        ...
        //Ixilium::Singleton<MapManager>::Instance().GetMap(m_creature->GetMapId(), m_creature)->GetPlayers().begin();
        /*
        list <Player*>::iterator i = sMapMgr.GetMap(m_creature->GetMapId(), m_creature)->GetPlayers().begin();

        for (i = sMapMgr.GetMap(m_creature->GetMapId(), m_creature)->GetPlayers().begin(); i != sMapMgr.GetMap(m_creature->GetMapId(), m_creature)->GetPlayers().end(); ++i)
        {
        AttackStart((*i));
        }
        */
        ...
        if (SpawnedAdds < 42)
        {
            ...
            if (AddSpawnTimer <= diff)
            {
                ...
                if (SpawnedAdds >= 42)
                {
                    ...
                    //Teleport self to a hiding spot (this causes errors in the Ixilium log but no real issues)
                    ...
                    else error_log("ISCR: Blackwing Lair: Unable to spawn nefarian properly.");
                }
