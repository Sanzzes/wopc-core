    ...
    boss_selin_fireheartAI(Creature* c) : ScriptedAI(c)
    {
        ...
        if (pInstance)
        {
            ...
            for (uint8 i = 0; i < size; ++i)
            {
                ...
                debug_log("ISCR: Selin: Adding Fel Crystal %u to list", guid);
    ...
    void Reset()
    {
        if (pInstance)
        {
            ...
            for (std::list<uint64>::iterator itr = Crystals.begin(); itr != Crystals.end(); ++itr)
            {
                ...
                {
                    if (!pUnit->isAlive())
                        CAST_CRE(pUnit)->Respawn();      // Let Ixilium handle setting death state, etc.
    ...
    void MovementInform(uint32 type, uint32 id)
    {
        if (type == POINT_MOTION_TYPE && id == 1)
        {
            ...
            else
            {
                ...
                error_log("ISCR: Selin Fireheart unable to drain crystal as the crystal is either dead or despawned");
                DrainingCrystal = false;
            }
        }
    }
        ...
        if (!DrainingCrystal)
        {
            ...
            if (maxPowerMana && ((m_creature->GetPower(POWER_MANA)*100 / maxPowerMana) < 10))
            {
                if (DrainCrystalTimer <= diff)
                {
                    ...
                    if (!RegularMode) DrainCrystalTimer = 10000 + rand()%5000;
