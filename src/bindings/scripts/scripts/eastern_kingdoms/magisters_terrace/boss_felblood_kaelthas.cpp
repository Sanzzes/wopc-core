    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        switch(Phase)
        {
            case 0:
            {
                ...
                {
                    DoCast(m_creature->getVictim(), RegularMode ? SPELL_FIREBALL_HEROIC : SPELL_FIREBALL_NORMAL);
...
struct SCRIPTS_DLL_DECL mob_felkael_flamestrikeAI : public ScriptedAI
{
    ...
    void UpdateAI(const uint32 diff)
    {
        if (FlameStrikeTimer <= diff)
        {
            DoCast(m_creature, RegularMode ? SPELL_FLAMESTRIKE1_HEROIC : SPELL_FLAMESTRIKE1_NORMAL, true);
