...
struct SCRIPTS_DLL_DECL instance_dark_portal : public ScriptedInstance
{
    ...
    void SetData(uint32 type, uint32 data)
    {
        ...
        case TYPE_MEDIVH:
            ...
            else
            {
                if (data == IN_PROGRESS)
                {
                    debug_log("ISCR: Instance Dark Portal: Starting event.");
                    InitWorldState();
                ...
                if (data == DONE)
                {
                    ...
                    debug_log("ISCR: Instance Dark Portal: Event completed.");
    ...
    Creature* SummonedPortalBoss(Creature* m_creature)
    {
        ...
        debug_log("ISCR: Instance Dark Portal: Summoning rift boss entry %u.",entry);
        ...
        debug_log("ISCR: Instance Dark Portal: What just happened there? No boss, no loot, no fun...");
        return NULL;
    }
    ...
    void DoSpawnPortal()
    {
        if (Creature* pMedivh = instance->GetCreature(MedivhGUID))
        {
            ...
            debug_log("ISCR: Instance Dark Portal: Creating Time Rift at locationId %i (old locationId was %u).",tmp,CurrentRiftId);
