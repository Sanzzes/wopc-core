...
{
    instance_culling_of_stratholme(Map* pMap) : ScriptedInstance(pMap), MalGanisChest(NULL) {Initialize();};
    ...
    void SetData(uint32 type, uint32 data)
    {
        switch(type)
        {
            ...
            case DATA_MAL_GANIS_EVENT:
                ...
                if (data == DONE && MalGanisChest)
                    MalGanisChest->SetRespawnTime(MalGanisChest->GetRespawnDelay());
                break;
