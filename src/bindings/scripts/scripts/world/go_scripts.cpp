...
//TODO: use gossip option ("Transcript the Tablet") instead, if Ixilium adds support.
        ...
        if (!pCreature->IsHostileTo(pPlayer))
        {
            ...
            if (FactionTemplateEntry const* pFaction = pCreature->getFactionTemplateEntry())
            {
                ...
                if (Spell)
                    ...
                    error_log("ISCR: go_ethereum_prison summoned Creature (entry %u) but faction (%u) are not expected by script.", pCreature->GetEntry(), pCreature->getFaction());
            }
        }
    }
