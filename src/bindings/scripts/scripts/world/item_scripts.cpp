...
bool ItemUse_item_flying_machine(Player* pPlayer, Item* pItem, SpellCastTargets const& targets)
{
    ...
    debug_log("ISCR: Player attempt to use item %u, but did not meet riding requirement",itemId);
...
bool ItemUse_item_trident_of_nazjan(Player* pPlayer, Item* _Item, SpellCastTargets const& targets)
{
	if (pPlayer->GetQuestStatus(11626) == QUEST_STATUS_INCOMPLETE)
	{
		if (Creature* Leviroth = pPlayer->FindNearestCreature(26452, 30.0f, true))
		{
			if (Leviroth->GetHealth()*100 / Leviroth->GetMaxHealth() >= 95)
			{
                Leviroth->SetHealth(urand(6000, 9000));
			} else pPlayer->SendEquipError(EQUIP_ERR_CANT_DO_RIGHT_NOW,_Item,NULL);
			return false;
	    } else pPlayer->SendEquipError(EQUIP_ERR_OUT_OF_RANGE,_Item,NULL);
		return false;
	} else pPlayer->SendEquipError(EQUIP_ERR_CANT_DO_RIGHT_NOW,_Item,NULL);
	return false;
}
