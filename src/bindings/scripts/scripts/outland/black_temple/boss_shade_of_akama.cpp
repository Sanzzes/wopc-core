...
struct SCRIPTS_DLL_DECL boss_shade_of_akamaAI : public ScriptedAI
{
    ...
    void IncrementDeathCount(uint64 guid = 0)               // If guid is set, will remove it from list of sorcerer
    {
        ...
        debug_log("ISCR: Increasing Death Count for Shade of Akama encounter");
        ++DeathCount;
    ...
    void FindChannelers()
    {
        ...
        if (!ChannelerList.empty())
        {
            for (std::list<Creature*>::iterator itr = ChannelerList.begin(); itr != ChannelerList.end(); ++itr)
            {
                ...
                debug_log("ISCR: Shade of Akama Grid Search found channeler %u. Adding to list", (*itr)->GetGUID());
            }
        }
