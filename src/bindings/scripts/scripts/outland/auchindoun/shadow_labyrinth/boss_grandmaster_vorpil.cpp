...
struct SCRIPTS_DLL_DECL mob_voidtravelerAI : public ScriptedAI
{
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        if (move <= diff)
        {
            if (sacrificed)
            {
                SpellEntry *spell = GET_SPELL(RegularMode?H_SPELL_EMPOWERING_SHADOWS:SPELL_EMPOWERING_SHADOWS);
                if (spell)
...
struct SCRIPTS_DLL_DECL boss_grandmaster_vorpilAI : public ScriptedAI
{
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        if (!RegularMode && banish_Timer <= diff)
        {
        ...
        if (DrawShadows_Timer <= diff)
        {
            ...
            DoCast(m_creature,RegularMode?H_SPELL_RAIN_OF_FIRE:SPELL_RAIN_OF_FIRE);
