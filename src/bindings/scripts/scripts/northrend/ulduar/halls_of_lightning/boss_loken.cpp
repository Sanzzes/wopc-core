...
struct SCRIPTS_DLL_DECL boss_lokenAI : public ScriptedAI
{
    ...
    void JustDied(Unit* pKiller)
    {
        ...
        if (!RegularMode && EncounterTime <= MAX_ENCOUNTER_TIME)
        {
    ...
    void UpdateAI(const uint32 uiDiff)
    {
                    ...
                    for (Map::PlayerList::const_iterator i = PlayerList.begin(); i != PlayerList.end(); ++i)
                        if (i->getSource() && i->getSource()->isAlive() && i->getSource()->isTargetableForAttack())
                        {
                            ...
                            if (m_fDist <= 1.0f) // Less than 1 yard
                                dmg = (RegularMode ? 150 : 100); // need to correct damage
                            else // Further from 1 yard
                                dmg = ((RegularMode ? 150 : 100) * m_fDist) + (RegularMode ? 150 : 100); // need to correct damage

                            m_creature->CastCustomSpell(i->getSource(), (RegularMode ? 59837 : 52942), &dmg, 0, 0, false);
                        }
                }
                m_uiPulsingShockwave_Timer = 2000;
        ...
        else
        {
            if (m_uiResumePulsingShockwave_Timer <= uiDiff)
            {
                ...
                DoCast(m_creature, SPELL_PULSING_SHOCKWAVE_AURA, true);
                  ...
                  DoCast(m_creature, RegularMode ? SPELL_PULSING_SHOCKWAVE_H : SPELL_PULSING_SHOCKWAVE_N); // need core support
                m_bIsAura = true;
        ...
        if (m_uiLightningNova_Timer <= uiDiff)
        {
            ...
            DoCast(m_creature, RegularMode ? SPELL_LIGHTNING_NOVA_H : SPELL_LIGHTNING_NOVA_N);

            m_bIsAura = false;
            m_uiResumePulsingShockwave_Timer = (RegularMode ? 4000 : 5000); // Pause Pulsing Shockwave aura
