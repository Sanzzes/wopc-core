...
struct SCRIPTS_DLL_DECL boss_volkhanAI : public ScriptedAI
{
    ...
    void JustDied(Unit* pKiller)
    {
        ...
        if (!RegularMode && GolemsShattered < 5)
        {
    ...
    void ShatterGolem()
    {
        ...
        for (std::list<uint64>::iterator itr = m_lGolemGUIDList.begin(); itr != m_lGolemGUIDList.end(); ++itr)
        {
            if (Creature* pTemp = Unit::GetCreature(*m_creature, *itr))
            {
                ...
                {
                    pTemp->CastSpell(pTemp, RegularMode ? SPELL_SHATTER_H : SPELL_SHATTER_N, false);
                    GolemsShattered += 1;
                }
            }
        }
    ...
    void JustSummoned(Creature* pSummoned)
    {
        if (pSummoned->GetEntry() == NPC_MOLTEN_GOLEM)
        {
            ...
            pSummoned->CastSpell(pSummoned, RegularMode ? SPELL_HEAT_H : SPELL_HEAT_N, false, NULL, NULL, m_creature->GetGUID());
        }
    }
    ...
    void UpdateAI(const uint32 uiDiff)
    {
        ...
        if (!m_bHasTemper && m_uiHealthAmountModifier >= 3)
        {
            if (m_uiShatteringStomp_Timer <= uiDiff)
            {
                ...
                DoCast(m_creature, RegularMode ? SPELL_SHATTERING_STOMP_H : SPELL_SHATTERING_STOMP_N);
...
struct SCRIPTS_DLL_DECL mob_molten_golemAI : public ScriptedAI
{
    ...
    void UpdateAI(const uint32 uiDiff)
    {
        ...
        {
            DoCast(m_creature->getVictim(), RegularMode ? SPELL_IMMOLATION_STRIKE_H : SPELL_IMMOLATION_STRIKE_N);
            m_uiImmolation_Timer = 5000;
        }
        else
