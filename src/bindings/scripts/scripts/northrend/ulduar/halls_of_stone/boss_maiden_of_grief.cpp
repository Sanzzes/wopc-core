...
struct SCRIPTS_DLL_DECL boss_maiden_of_griefAI : public ScriptedAI
{
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        {
            DoCast(m_creature->getVictim(), RegularMode ? SPELL_STORM_OF_GRIEF_H : SPELL_STORM_OF_GRIEF_N, true);
        ...
        if (ShockOfSorrowTimer <= diff)
        {
            ...
            DoCast(m_creature, RegularMode ? SPELL_SHOCK_OF_SORROW_H : SPELL_SHOCK_OF_SORROW_N);
        ...
        if (PillarOfWoeTimer <= diff)
        {
            ...
            if(pTarget)
                DoCast(pTarget, RegularMode ? SPELL_PILLAR_OF_WOE_H : SPELL_PILLAR_OF_WOE_N);
            else
                DoCast(m_creature->getVictim(), RegularMode ? SPELL_PILLAR_OF_WOE_H : SPELL_PILLAR_OF_WOE_N);
    ...
    void JustDied(Unit* killer)
    {
        ...
        if (!RegularMode && AchievTimer < 60000 && pMap && pMap->IsDungeon() && AchievGoodGrief)
        {
