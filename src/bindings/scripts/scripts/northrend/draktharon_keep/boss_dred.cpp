...
struct SCRIPTS_DLL_DECL boss_dredAI : public ScriptedAI
{
    ...
    void JustDied(Unit* killer)
    {
        if (pInstance)
        {
            ...
            if (!RegularMode && pInstance->GetData(DATA_KING_DRED_ACHIEV) == 6)
...
struct SCRIPTS_DLL_DECL npc_drakkari_gutripperAI : public ScriptedAI
{
    ...
    void JustDied(Unit* killer)
    {
        if (pInstance)
        {
            if (m_creature->GetMap()->IsRegularDifficulty() && pInstance->GetData(DATA_DRED_EVENT) == IN_PROGRESS && pInstance->GetData(DATA_KING_DRED_ACHIEV) < 6)
            {
...
struct SCRIPTS_DLL_DECL npc_drakkari_scytheclawAI : public ScriptedAI
{
