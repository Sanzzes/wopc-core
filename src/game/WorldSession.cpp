...
m_latency(0), m_TutorialsChanged(false)
{
    if (sock)
    {
        ...
        sock->AddReference ();
        loginDatabase.PExecute("UPDATE account_settings SET active_logon_id = 1 WHERE id = %u;", GetAccountId());
    }
}
...
WorldSession::~WorldSession()
{
    ...
    loginDatabase.PExecute("UPDATE account_settings SET active_logon_id = 0 WHERE id = %u;", GetAccountId());
