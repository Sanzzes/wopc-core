...
pAuraHandler AuraHandler[TOTAL_AURAS]=
{
    ...
    &Aura::HandleNoImmediateEffect,                         //163 SPELL_AURA_MOD_CRIT_DAMAGE_BONUS_MELEE      implemented in Unit::CalculateMeleeDamage and Unit::SpellCriticalDamageBonus
    ...
    &Aura::HandleNoImmediateEffect,                         //205 SPELL_AURA_MOD_ATTACKER_SPELL_CRIT_DAMAGE 
        ...
        if ( !source->hasUnitState(UNIT_STAT_ISOLATED) )
        {
            ...
            for (std::list<Unit*>::iterator tIter = targets.begin(); tIter != targets.end(); tIter++)
            {
                ...
                else
                {
                    ...
                    {
                        if (!sSpellMgr.CanAurasStack(GetSpellProto(), iter->second->GetSpellProto(), iter->second->GetCasterGUID() == GetCasterGUID()))
                        {
                            skip = true;
                            break;
                        }
            ...
            switch(m_spellProto->SpellFamilyName)
            {
                ...
                case SPELLFAMILY_WARLOCK:
                    ...
                    else if (m_spellProto->SpellFamilyFlags[0] & 0x00004000 && GetAuraName() == SPELL_AURA_PERIODIC_DAMAGE)
                    {
                        ...
                        if (m_target->GetMaxHealth() <=  m_target->GetHealth()*0.25f)
                            m_amount *= 4;
                    }
                    break;
                case SPELLFAMILY_PRIEST:
                {
                    ...
                    // Borrowed Time
                    if (m_spellProto->SpellFamilyFlags[0] & 0x1 && GetAuraName() == SPELL_AURA_SCHOOL_ABSORB)
                    {
...
void Aura::_RemoveAura()
{
    ...
    {
        if (uint32 customAttr = sSpellMgr.GetSpellCustomAttr(id))
        {
    ...
    switch (modOp)
    {
        ...
        case SPELLMOD_EFFECT3:
        {
            ...
            for (Unit::AuraMap::iterator iter = auras.begin(); iter != auras.end(); ++iter)
            {
                ...
                if (aur->IsPassive() && aur->GetCasterGUID() == guid && sSpellMgr.IsAffectedByMod(aur->GetSpellProto(), m_spellmod))
                {
        ...
        switch(auraSpellInfo->SpellFamilyName)
        {
            ...
            case SPELLFAMILY_SHAMAN:
            {
                switch(auraId)
                {
                    ...
                    // Feral Spirit Spirit Walk Trigger
                    case 58875: trigger_spell_id = 58876; break;
                    default:
                        break;
                }
                break;
        ...
        switch(m_spellProto->SpellFamilyName)
        {
            ...
            case SPELLFAMILY_WARLOCK:
                ...
                if (m_spellProto->SpellFamilyFlags[1] & 0x40000)
                {
                    ...
                    if (caster)
                        m_target->CastCustomSpell(caster, 48210, &bp0, 0, 0, true, NULL, this);
                    return;
                }
                break;
            case SPELLFAMILY_PRIEST:
...
void AuraEffect::HandleAuraModShapeshift(bool apply, bool Real, bool changeAmount)
{
    ...
    if (form == FORM_TRAVEL && ((Player*)m_target)->HasAura(52610))
        m_target->CastSpell(m_target, 62071, true);

    if (form == FORM_TREE && ((Player*)m_target)->HasAura(52610))
        m_target->CastSpell(m_target, 62071, true);

    if (form == FORM_AQUA && ((Player*)m_target)->HasAura(52610))
        m_target->CastSpell(m_target, 62071, true);

    if (form == FORM_BEAR && ((Player*)m_target)->HasAura(52610))
        m_target->CastSpell(m_target, 62071, true);

    if (form == FORM_DIREBEAR && ((Player*)m_target)->HasAura(52610))
        m_target->CastSpell(m_target, 62071, true);

    if (form == FORM_MOONKIN && ((Player*)m_target)->HasAura(52610))
        m_target->CastSpell(m_target, 62071, true);

    if (form == FORM_FLIGHT && ((Player*)m_target)->HasAura(52610))
        m_target->CastSpell(m_target, 62071, true);

    if (form == FORM_FLIGHT_EPIC && ((Player*)m_target)->HasAura(52610))
        m_target->CastSpell(m_target, 62071, true);
...
void AuraEffect::HandleModMechanicImmunity(bool apply, bool Real, bool /*changeAmount*/)
{
    ...
    // Spirit Walk -- feral spirit -- missing movement impairing effects immunity
    if (GetId() == 58876 && 58875)
    {
        if (apply)
            m_target->RemoveMovementImpairingAuras();
        ...
        case SPELLFAMILY_GENERIC:
        {
            switch(spell->Id)
            {
                case 62399: // Overload Circuit
                    if(m_target->GetMap()->IsDungeon() && m_target->GetAuras().count(62399) >= (m_target->GetMap()->IsRaidOrHeroicDungeon() ? 4 : 2))
                     {
