...
AccountOpResult AccountMgr::CreateAccount(std::string username, std::string password)
{
    ...
    if (!loginDatabase.PExecute("INSERT INTO account_settings(username, sha_pass_hash, joindate) VALUES('%s', '%s', NOW())",
        username.c_str(), CalculateShaPassHash(username, password).c_str()))
        return AOR_DB_INTERNAL_ERROR;                       // unexpected error

    loginDatabase.Execute("INSERT INTO realmcharacters (realmid, acctid, numchars) SELECT logonlist.id, account_settings.id, 0 FROM logonlist, account_settings LEFT JOIN realmcharacters ON acctid = account_settings.id WHERE acctid IS NULL");
...
AccountOpResult AccountMgr::ChangeUsername(uint32 accid, std::string new_uname, std::string new_passwd)
{
    ...
    if (!loginDatabase.PExecute("UPDATE account_settings SET v = '0', s = '0', username = '%s', sha_pass_hash = '%s' WHERE id = '%d'",
...
AccountOpResult AccountMgr::ChangePassword(uint32 accid, std::string new_passwd)
{
    ...
    if (!loginDatabase.PExecute("UPDATE account_settings SET v = '0', s = '0', sha_pass_hash = '%s' WHERE id = '%d'",
...
uint32 AccountMgr::GetId(std::string username)
{
    ...
    QueryResult *result = loginDatabase.PQuery("SELECT id FROM account_settings WHERE username = '%s'", username.c_str());
    if (!result)
        return 0;
    else
    {
...
{
    QueryResult *result = loginDatabase.PQuery("SELECT username FROM account_settings WHERE id = '%u'", acc_id);
    if (result)
    {
...
bool AccountMgr::CheckPassword(uint32 accid, std::string passwd)
{
    ...
    QueryResult *result = loginDatabase.PQuery("SELECT 1 FROM account_settings WHERE id = '%d' AND sha_pass_hash = '%s'", accid, CalculateShaPassHash(username, passwd).c_str());
    if (result)
    {
        delete result;
        return true;
