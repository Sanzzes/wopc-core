...
bool ChatHandler::HandleReloadPoiCommand(const char *args)
{
    // Reload the Quest Poi Strings
    sLog.outString( "Re-Loading POi's..." );
    sObjectMgr.LoadQuestPOI();
    SendGlobalGMSysMessage("POI's reloaded.");
    return true;
}
...
bool ChatHandler::HandleAccountSetGmLevelCommand(const char *args)
{
    ...
    if (getSelectedPlayer() && arg1 && !arg2)
    {
        ...
        loginDatabase.PExecute("UPDATE account_access SET gmlevel = '%d' WHERE id = '%u'", gm, targetAccountId);
        return true;
    }
    else
    {
...
bool ChatHandler::HandleNpcInfoCommand(const char* /*args*/)
{
    ...
    QueryResult *result = WorldDatabase.PQuery( "SELECT `ScriptName`, `AIName` FROM `creature_template` WHERE `entry` = '%u'", Entry);
    Field *fields = result->Fetch();
    ...
    const char* cName = cInfo->Name;
    const char* cScript = fields[0].GetString();
    const char* aiScript = fields[1].GetString();
    uint32 cPhase = target->GetPhaseMask();
    uint32 cFACa = cInfo->faction_A;
    uint32 cFACh = cInfo->faction_H;
    const uint32 model_1 = cInfo->Modelid1;
    const uint32 model_2 = cInfo->Modelid2;
    const uint32 model_3 = cInfo->Modelid3;
    const uint32 model_4 = cInfo->Modelid4;
    ...
    PSendSysMessage(LANG_NPCINFO_SHOW_SCRIPTNAME, cName, cScript);
...
bool ChatHandler::HandleBanListAccountCommand(const char *args)
{
    ...
    if (filter.empty())
    {
        result = loginDatabase.Query("SELECT account.id, username FROM account_settings, account_banned"
            " WHERE account.id = account_banned.id AND active = 1 GROUP BY account.id");
    }
    else
    {
        result = loginDatabase.PQuery("SELECT account.id, username FROM account_settings, account_banned"
...
bool ChatHandler::HandleBanListHelper(QueryResult* result)
{
    ...
    if (m_session)
    {
        do
        {
            ...
            QueryResult* banresult = loginDatabase.PQuery("SELECT account.username FROM account_settings,account_banned WHERE account_banned.id='%u' AND account_banned.id=account.id",accountid);
            if (banresult)
            {
        ...
        for (Player::BoundInstancesMap::const_iterator itr = binds.begin(); itr != binds.end(); ++itr)
        {
            ...
            if (const MapEntry* entry = sMapStore.LookupEntry(itr->first))
            {
                PSendSysMessage("map: %d (%s) inst: %d perm: %s diff: %s canReset: %s TTR: %s",
                    ...
                    save->GetDungeonDifficulty() == DUNGEON_DIFFICULTY_NORMAL ? "normal" : "heroic", save->CanReset() ? "yes" : "no", timeleft.c_str());
            }
            else
                PSendSysMessage("bound for a nonexistant map %u", itr->first);            counter++;
        }
    }
...
bool ChatHandler::HandleInstanceUnbindCommand(const char *args)
{
    ...
    for (uint8 i = 0; i < MAX_DIFFICULTY; ++i)
    {
        ...
        for (Player::BoundInstancesMap::iterator itr = binds.begin(); itr != binds.end();)
        {
			...
			//Prevent freeze... Do not unbind if Player is in Instance
			if (itr->second.save->GetMapId() == player->GetMapId())
			{
				PSendSysMessage("Can't unbind in Instance Area!");
				return true;
			}
...
bool ChatHandler::HandleGMListFullCommand(const char* /*args*/)
{
    ...
    QueryResult *result = loginDatabase.Query( "SELECT a.username,aa.gmlevel FROM account_settings a, account_access aa WHERE a.id=aa.id AND aa.gmlevel > 0" );
    if (result)
    {
...
bool ChatHandler::HandleAccountSetAddonCommand(const char *args)
{
    ...
    // No SQL injection
    loginDatabase.PExecute("UPDATE account_settings SET expansion = '%d' WHERE id = '%u'",expansion,account_id);
