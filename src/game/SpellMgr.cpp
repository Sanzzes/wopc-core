...
void SpellMgr::LoadSpellTargetPositions()
{
    ...
    for (uint32 i = 1; i < sSpellStore.GetNumRows(); ++i)
    {
        ...
        if (found)
        {
			...
			{
				sLog.outDebug("Spell (ID: %u) does not have record in `spell_target_position`, creating seat in table.", i);
				WorldDatabase.PExecute("INSERT INTO `spell_target_position` (`id`)VALUES('%u')", i);
			}
        }
    }
...
void SpellMgr::LoadSpellScriptTarget()
{
    ...
    do
    {
        ...
        for (int i = 0; i < 3; ++i)
        {
            if( spellProto->EffectImplicitTargetA[i] == TARGET_UNIT_NEARBY_ENTRY ||
                ...
                spellProto->EffectImplicitTargetA[i] == TARGET_OBJECT_USE ||
                spellProto->EffectImplicitTargetB[i] == TARGET_OBJECT_USE )
            {
                targetfound = true;
                break;
            }
        ...
        for (int i = 0; i < 3; ++i)
        {
            if (spellProto->EffectImplicitTargetA[i] == TARGET_OBJECT_USE ||
                spellProto->EffectImplicitTargetB[i] == TARGET_OBJECT_USE)
            {
    ...
    for (uint32 i = 1; i < sSpellStore.GetNumRows(); ++i)
    {
        ...
        if (found)
        {
            SpellScriptTargetBounds bounds = sSpellMgr.GetSpellScriptTargetBounds(i);
    ...
    do
    {
        ...
        if(spell)
        {
            ...
            if(familyMaskA != UI64LIT(0xFFFFFFFFFFFFFFFF) || familyMaskB != 0xFFFFFFFF)
            {
                ...
                {
					if(spellEntry->SpellFamilyFlags != 0 || spellEntry->SpellFamilyFlags[2] != 0)
                    {
                ...
                {
                    if((spellEntry->SpellFamilyFlags[0] & familyMaskA)==0 && (spellEntry->SpellFamilyFlags[1] & familyMaskB)==0)
                    {
                        sLog.outError("Spell %u '%s' not fit to (" I64FMT "," I32FMT ") but used in %s.",spell,name.c_str(),familyMaskA,familyMaskB,code.c_str());
                        continue;
                    }
        ...
        else
        {
            ...
            for(uint32 spellId = 1; spellId < sSpellStore.GetNumRows(); ++spellId)
            {
                ...
                if(familyMaskA != UI64LIT(0xFFFFFFFFFFFFFFFF) || familyMaskB != 0xFFFFFFFF)
                {
                    if(familyMaskA == UI64LIT(0x0000000000000000) && familyMaskB == 0x00000000)
                    {
                        if(spellEntry->SpellFamilyFlags != 0 || spellEntry->SpellFamilyFlags != 0)
                            continue;
                    }
                    else
                    {
...
{
    SpellScriptTargetBounds bounds = sSpellMgr.GetSpellScriptTargetBounds(30531);
