...
void WorldSession::HandleLfdPlayerLockInfoRequest( WorldPacket & recv_data )
{
    sLog.outDebug("CMSG_LFD_PLAYER_LOCK_INFO_REQUEST");
    recv_data.hexlike();

    /*sLog.outDebug( "WORLD: Recv CMSG_LFD_PLAYER_LOCK_INFO_REQUEST" );

    WorldPacket data(CMSG_LFD_PLAYER_LOCK_INFO_REQUEST, 4+4*count);
    data << count;
