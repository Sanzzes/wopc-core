...
Quest::Quest(Field * questRecord)
{
    ...
    RewRepFactionAlliance = questRecord[96].GetUInt32();
    RewRepFactionHorde = questRecord[97].GetUInt32();

    for (int i = 0; i < QUEST_REPUTATIONS_COUNT; ++i)
        RewRepValue[i] = questRecord[98+i].GetInt32();

    RewRepValueAlliance = questRecord[103].GetInt32();
    RewRepValueHorde = questRecord[104].GetInt32();
    RewHonorableKills = questRecord[105].GetUInt32();
...
uint32 Quest::XPValue( Player *pPlayer ) const
{
    if ( pPlayer )
    {
        if ( RewMoneyMaxLevel > 0 )
        {
            ...
            int32 qLevel = QuestLevel > 0 ? (uint32)QuestLevel : 0;
            float fullxp = 0;
            if (qLevel >= 15)
