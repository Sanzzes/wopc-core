...
void AuctionHouseBot::addNewAuctions(Player *AHBplayer, AHBConfig *config)
{
    ...
    if (debug_Out) sLog.outString("AHSeller: %u count", cnt);
        ...
        while (itemID == 0 && loopbreaker <= 50)
        {
            ...
            ItemPrototype const* prototype = ObjectMgr::GetItemPrototype(itemID);
            if (prototype == NULL)
            {
            ...
            uint32 dep = AuctionHouseMgr::GetAuctionDeposit(ahEntry, etime, item);
...
void AuctionHouseBot::addNewAuctionBuyerBotBid(Player *AHBplayer, AHBConfig *config, WorldSession *session)
{
    ...
    for (uint32 count = 1; count <= config->GetBidsPerInterval(); ++count)
    {
        ...
        // get item prototype
        ItemPrototype const* prototype = ObjectMgr::GetItemPrototype(auction->item_template);
...
void AuctionHouseBot::Update()
{
    ...
    /*
    AuctionHouseObject* auctionHouse1 = sAuctionMgr.GetAuctionsMap(55);
    AuctionHouseObject* auctionHouse2 = sAuctionMgr.GetAuctionsMap(29);
    AuctionHouseObject* auctionHouse3 = sAuctionMgr.GetAuctionsMap(120);
...
void AuctionHouseBot::IncrementItemCounts(AuctionEntry* ah)
{
    ...
    // get item prototype
    ItemPrototype const* prototype = ObjectMgr::GetItemPrototype(ah->item_template);
...
{
    // get item prototype
    ItemPrototype const* prototype = ObjectMgr::GetItemPrototype(item_template);
