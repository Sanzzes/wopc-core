...
bool ChatHandler::HandleGMTicketAssignToCommand(const char* args)
{
    ...
    QueryResult *result = loginDatabase.PQuery("SELECT `gmlevel` FROM `account_access` WHERE `id` = '%u'", accid);
...
bool ChatHandler::HandleGMTicketDeleteByIdCommand(const char* args)
{
	...
	HandleGMTicketUnAssignCommand(args);
	HandleGMTicketCloseByIdCommand(args);
