...
void ObjectMgr::LoadGameobjects()
{
    ...
    do
    {
        ...
        if (data.spawnMask == 0 && ~spawnMasks[data.mapid])
...
void ObjectMgr::LoadItemPrototypes(bool reload)
{
    ...
    for (uint32 i = 1; i < sItemStorage.MaxEntry; ++i)
    {
        ...
        if (dbcitem)
        {
            ...
            {
                sLog.outErrorDb("Item (Entry: %u) not correct class %u, must be %u (using and applying to DB).",i,proto->Class,dbcitem->Class);
                // Update with newer proper info then update DB aswell.
                const_cast<ItemPrototype*>(proto)->Class = dbcitem->Class;
                WorldDatabase.PExecute("UPDATE `item_template` SET `class` = '%u' WHERE `entry` = '%u'", i,proto->Class,dbcitem->Class);
            }
            ...
            */
            ...
            {
                sLog.outErrorDb("Item (Entry: %u) not correct %i Unk0, must be %i (using and applying to DB).",i,proto->Unk0,dbcitem->Unk0);
                const_cast<ItemPrototype*>(proto)->Unk0 = dbcitem->Unk0;
                WorldDatabase.PExecute("UPDATE `item_template` SET `Unk0` = '%i' WHERE `entry` = '%u'", dbcitem->Unk0,i);
            }

            if (proto->Material != dbcitem->Material)
            {
                sLog.outErrorDb("Item (Entry: %u) not correct %i material, must be %i (using and applying to DB).",i,proto->Material,dbcitem->Material);
                const_cast<ItemPrototype*>(proto)->Material = dbcitem->Material;
                WorldDatabase.PExecute("UPDATE `item_template` SET `Material` = '%i' WHERE `entry` = '%u'", dbcitem->Material,i);
            }

            if (proto->InventoryType != dbcitem->InventoryType)
            {
                sLog.outErrorDb("Item (Entry: %u) not correct %u inventory type, must be %u (using and applying to DB).",i,proto->InventoryType,dbcitem->InventoryType);
                const_cast<ItemPrototype*>(proto)->InventoryType = dbcitem->InventoryType;
                WorldDatabase.PExecute("UPDATE `item_template` SET `InventoryType` = '%u' WHERE `entry` = '%u'", dbcitem->InventoryType,i);
            }

            if (proto->DisplayInfoID != dbcitem->DisplayId)
            {
                sLog.outErrorDb("Item (Entry: %u) not correct %u display id, must be %u (using and applying to DB).",i,proto->DisplayInfoID,dbcitem->DisplayId);
                const_cast<ItemPrototype*>(proto)->DisplayInfoID = dbcitem->DisplayId;
                WorldDatabase.PExecute("UPDATE `item_template` SET `displayid` = '%u' WHERE `entry` = '%u'", dbcitem->DisplayId,i);
            }
            if (proto->Sheath != dbcitem->Sheath)
            {
                sLog.outErrorDb("Item (Entry: %u) not correct %u sheath, must be %u  (using and applying to DB).",i,proto->Sheath,dbcitem->Sheath);
                const_cast<ItemPrototype*>(proto)->Sheath = dbcitem->Sheath;
                WorldDatabase.PExecute("UPDATE `item_template` SET `sheath` = '%u' WHERE `entry` = '%u'", dbcitem->Sheath,i);
            }
        }
        else
        ...
        {
            sLog.outErrorDb("Item (Entry: %u) has wrong Class value (%u), id is too high..",i,proto->Class);
            const_cast<ItemPrototype*>(proto)->Class = ITEM_CLASS_MISC;
        }

        if (proto->SubClass >= MaxItemSubclassValues[proto->Class])
        {
            sLog.outErrorDb("Item (Entry: %u) has wrong Subclass value (%u) for class (%u), id is too high..",i,proto->SubClass,proto->Class);
            const_cast<ItemPrototype*>(proto)->SubClass = 0;// exist for all item classes
        }

        if (proto->Quality >= MAX_ITEM_QUALITY)
        {
            sLog.outErrorDb("Item (Entry: %u) has wrong Quality value (%u), id is too high..",i,proto->Quality);
            const_cast<ItemPrototype*>(proto)->Quality = ITEM_QUALITY_NORMAL;
        }

        if (proto->BuyCount <= 0)
        {
            sLog.outErrorDb("Item (Entry: %u) has wrong BuyCount value (%u), set to default(1), Please update this value",i,proto->BuyCount);
...
void ObjectMgr::LoadPlayerInfo()
{
                ...
                if (!current_race || !current_class)
                {
                    ...
                    for (uint32 r = min_race; r < max_race; ++r)
                        for (uint32 c = min_class; c < max_class; ++c)
                            playerInfo[r][c].spell.push_back(spell_id);
                }
                else
                    playerInfo[current_race][current_class].spell.push_back(spell_id);
    ...
    // Loading levels data (class only dependent)
    {
        ...
        do
        {
            ...
            uint8 current_level = fields[1].GetUInt32();
            if (current_level > sWorld.getConfig(CONFIG_MAX_PLAYER_LEVEL))
            {
                if (current_level > STRONG_MAX_LEVEL)        // hardcoded level maximum
...
void ObjectMgr::LoadGMTickets()
{
    ...
    CharacterDatabase.PQuery("DELETE FROM gm_tickets WHERE closed = 1");
