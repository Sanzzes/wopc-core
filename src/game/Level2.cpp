...
bool ChatHandler::HandleMuteCommand(const char* args)
{
    ...
    loginDatabase.PExecute("UPDATE account_settings SET mutetime = " UI64FMTD " WHERE id = '%u'",uint64(mutetime), account_id );
...
bool ChatHandler::HandleUnmuteCommand(const char* args)
{
    ...
    loginDatabase.PExecute("UPDATE account_settings SET mutetime = '0' WHERE id = '%u'", account_id );
...
bool ChatHandler::HandleNpcChangeLevelCommand(const char* args)
{
    ...
    if (pCreature->isPet())
    {
        ...
        {
            pCreature->SetUInt32Value(UNIT_FIELD_PETNEXTLEVELEXP, sObjectMgr.GetXPForLevel(lvl)/5);
...
bool ChatHandler::HandlePInfoAccountCommand(const char* args)
{
    ...
    if (result)
    {
        ...
        QueryResult* resultgmlevel = loginDatabase.PQuery("SELECT gmlevel FROM account_access WHERE id = '%u' AND RealmID = '%d'", accId, realmID);

        if (resultgmlevel) {
            Field* fields = resultgmlevel->Fetch();
            security = fields[0].GetUInt32();
            delete resultgmlevel;
        }
...
bool ChatHandler::HandlePInfoIpCommand(const char* args)
{
    ...
    {	
        ...
        for (int i = 0; i < count ; i++)
        {
            ...
            active_logon_id = fields[4].GetUInt32();
            if (active_logon_id > 0)
            {
                isOnline = "(ONLINE) ";
            }
            else
...
bool ChatHandler::HandleLookupPlayerIpCommand(const char* args)
{
    ...
    QueryResult* result = loginDatabase.PQuery ("SELECT id,username FROM account_settings WHERE last_ip = '%s'", ip.c_str ());
...
bool ChatHandler::HandleLookupPlayerAccountCommand(const char* args)
{
    ...
    QueryResult* result = loginDatabase.PQuery ("SELECT id,username FROM account_settings WHERE username = '%s'", account.c_str ());
...
bool ChatHandler::HandleLookupPlayerEmailCommand(const char* args)
{
    ...
    QueryResult* result = loginDatabase.PQuery ("SELECT id,username FROM account_settings WHERE email = '%s'", email.c_str ());
...
bool ChatHandler::LookupPlayerSearchCommand(QueryResult* result, int32 limit)
{
    ...
    do
    {
        ...
        QueryResult* chars = CharacterDatabase.PQuery("SELECT guid,name FROM characters WHERE account_settings = '%u'", acc_id);
        if (chars)
        {
