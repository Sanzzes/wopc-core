...
World::AddSession_ (WorldSession* s)
{
    ...
    if (pLimit > 0)
    {
        ...
        popu /= pLimit;
        popu *= 2;
        loginDatabase.PExecute ("UPDATE logonlist SET population = '%f' WHERE id = '%d'", popu, realmID);
...
sLog.outError("DurabilityLossChance.Damage (%f) must be >= 0. Using 0.0 instead.",rate_values[RATE_DURABILITY_LOSS_DAMAGE]);
	...
	m_configs[CONFIG_CHATLEV] = sConfig.GetIntDefault("ChatLev",1);
...
void World::SetInitialWorldSettings()
{
    ...
    loginDatabase.PExecute("UPDATE logonlist SET icon = %u, timezone = %u WHERE id = '%d'", server_type, realm_zone, realmID);
    ...
    if (m_configs[CONFIG_CHECK_DB])
    {
        ...
        if (!sObjectMgr.CheckDB() || !sSpellMgr.CheckDB())
        {
    ...
    sLog.outString("STARTUPTIME: %i minutes and %i seconds", uStartInterval / 60000, (uStartInterval % 60000) / 1000);
}
...
BanReturn World::BanAccount(BanMode mode, std::string nameOrIP, std::string duration, std::string reason, std::string author)
{
    ...
    switch(mode)
    {
        case BAN_IP:
            ...
            resultAccounts = loginDatabase.PQuery("SELECT id FROM account_settings WHERE last_ip = '%s'",nameOrIP.c_str());
            loginDatabase.PExecute("INSERT INTO ip_banned VALUES ('%s',UNIX_TIMESTAMP(),UNIX_TIMESTAMP()+%u,'%s','%s')",nameOrIP.c_str(),duration_secs,safe_author.c_str(),reason.c_str());
            break;
        case BAN_ACCOUNT:
            //No SQL injection as string is escaped
            resultAccounts = loginDatabase.PQuery("SELECT id FROM account_settings WHERE username = '%s'",nameOrIP.c_str());
            break;
        case BAN_CHARACTER:
            //No SQL injection as string is escaped
...
{
    QueryResult *result = loginDatabase.PQuery("SELECT allowedSecurityLevel from logonlist WHERE id = '%d'", realmID);
    if (result)
    {
...
{
    QueryResult* result = WorldDatabase.Query("SELECT db_revision, script_version, cache_id, gixi_version FROM version LIMIT 1");
    if (result)
    {
        Field* fields = result->Fetch();

        m_DBVersion              = fields[0].GetCppString();  //DB-Revision
        m_CreatureEventAIVersion = fields[1].GetCppString();
        m_GIXIVersion            = fields[3].GetCppString();
    ...
    if (m_GIXIVersion.empty())
        m_GIXIVersion = "Unknown GIXI Version";
}
