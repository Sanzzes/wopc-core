...
bool ChatHandler::HandleGMListIngameCommand(const char* /*args*/)
{
    ...
    for (; itr != m.end(); ++itr)
    {
        ...
        if ((itr->second->isGameMaster() || (itr_sec > SEC_VIP && itr_sec <= sWorld.getConfig(CONFIG_GM_LEVEL_IN_GM_LIST))) &&
...
bool ChatHandler::HandleAccountLockCommand(const char* args)
{
    ...
    if (argstr == "on")
    {
        loginDatabase.PExecute( "UPDATE account_settings SET locked = '1' WHERE id = '%d'",m_session->GetAccountId());
    ...
    if (argstr == "off")
    {
        loginDatabase.PExecute( "UPDATE account_settings SET locked = '0' WHERE id = '%d'",m_session->GetAccountId());
