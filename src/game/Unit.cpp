...
bool Unit::RemoveNoStackAurasDueToAura(Aura *Aur)
{
    ...
    //bool linked = sSpellMgr.GetSpellCustomAttr(spellId) & SPELL_ATTR_CU_LINK_AURA? true : false;
    ...
    switch(dummySpell->SpellFamilyName)
    {
        ...
        case SPELLFAMILY_WARLOCK:
        {
            ...
            switch (dummySpell->Id)
            {
                ...
                case 63320:
                {
					if (triggeredByAura->GetSpellProto()->SpellIconID != 154 ||
						triggeredByAura->GetSpellProto()->SpellIconID != 208)
                        return false;
...
Unit* Unit::GetNextRandomRaidMemberOrPet(float radius)
{
    ...
    for (GroupReference *itr = pGroup->GetFirstMember(); itr != NULL; itr = itr->next())
        ...
        if (Target && Target != this && Target->isAlive() && IsWithinDistInMap(Target, radius) && !IsHostileTo(Target))
...
bool Unit::isTargetableForAttack(bool inverseAlive /*=false*/) const
{
    ...
    return IsInWorld() && isAttackableByAOE() && !hasUnitState(UNIT_STAT_DIED) && !isInFlight() /*&& !isStealth()*/;
}
...
void Unit::BuildMovementPacket(ByteBuffer *data) const
{
    switch(GetTypeId())
    {
        ...
        case TYPEID_PLAYER:
            ...
            if (isInFlight())
            {
                ASSERT(const_cast<Unit*>(this)->GetMotionMaster()->GetCurrentMovementGeneratorType() == FLIGHT_MOTION_TYPE);
