...
void PetAI::UpdateAI(const uint32 diff)
{
	...
	m_oldReactState = m_creature->GetReactState();
	
    // Stop Attacking if target has sleeping or disorientiring aura (stun)
    if(Unit* pVictim = m_creature->getVictim())
    {
        if(pVictim->HasAuraType(SPELL_AURA_MOD_STUN) // may be not all include:(
        || pVictim->HasAuraType(SPELL_AURA_MOD_CONFUSE))
        {  
    ...
    {   // select unit, that attack hunter's pet or hunter (cannot be stunned unit) and start attack it
        if(Unit* pVictim = m_creature->SelectNearestTarget(30.0f))
        if(!pVictim->HasAuraType(SPELL_AURA_MOD_STUN) && !pVictim->HasAuraType(SPELL_AURA_MOD_CONFUSE))
        if(pVictim->Attack(m_creature,true) || pVictim->Attack(me->GetCharmerOrOwner(),true))
        {
            m_creature->SetReactState(m_oldReactState);
            m_creature->CombatStart(pVictim);            
