...
void WorldSession::HandleAreaSpiritHealerQueryOpcode( WorldPacket & recv_data )
{
    ...
    /*if (bg)
    {
        unit->SendAreaSpiritHealerQueryOpcode(_player, bg, guid);
    }
    else
...
void WorldSession::SendBattleGroundOrArenaJoinError(uint8 err)
{
    ...
    ChatHandler::FillMessageData(&data, NULL, CHAT_MSG_BG_SYSTEM_NEUTRAL, LANG_UNIVERSAL, NULL, 0, GetWOPCCOREString(msg), NULL);
    SendPacket(&data);
    return;
}
