    ...
    if (unitTarget && unitTarget->isAlive())
    {
        switch (m_spellInfo->SpellFamilyName)
        {
            ...
            case SPELLFAMILY_WARLOCK:
            {
                ...
                else if (m_spellInfo->TargetAuraState == AURA_STATE_CONFLAGRATE)
                {
                    ...
                    if (aura)
                    {
                        ...
                        damage += pdamage * aura->GetTotalTicks() * 60 /100; // 60 %
            ...
            case SPELLFAMILY_PRIEST:
            {
                ...
                else if (m_caster->m_form == FORM_SHADOW && (m_spellInfo->SpellFamilyFlags && 0x00002000))
                {
            ...
            case SPELLFAMILY_HUNTER:
            {
                ...
                // Mongoose Bite
                else if ((m_spellInfo->SpellFamilyFlags[1] & 0x000000002) && m_spellInfo->SpellVisual[0] == 342)
                {
                ...
                // Arcane Shot
                else if ((m_spellInfo->SpellFamilyFlags[1] & 0x00000800) && m_spellInfo->maxLevel > 0)
                {
...
void Spell::EffectTriggerSpell(uint32 effIndex)
{
    ...
    switch(triggered_spell_id)
    {
        ...
        // Shaman Feral Spirits need apply healing trigger aura on pet
        case 58879:
        {
            if (Unit *pet = unitTarget->GetGuardianPet())
...
void Spell::EffectPersistentAA(uint32 i)
{
    ...
    if (!dynObj->Create(sObjectMgr.GenerateLowGuid(HIGHGUID_DYNAMICOBJECT), caster, m_spellInfo->Id, 1<<i, m_targets.m_dstPos, duration, radius, false))
    {
        delete dynObj;
        return;
    }
        ...
        default:
            ...
            switch (properties->Type)
            {
                ...
                case SUMMON_TYPE_TOTEM:
                {
                    ...
                    {
                        //summon->SendCreateUpdateToPlayer((Player*)m_originalCaster);
...
void Spell::EffectAddFarsight(uint32 i)
{
    ...
    int32 duration = GetSpellDuration(m_spellInfo);
    DynamicObject* dynObj = new DynamicObject;
    if (!dynObj->Create(sObjectMgr.GenerateLowGuid(HIGHGUID_DYNAMICOBJECT), m_caster, m_spellInfo->Id, 0, m_targets.m_dstPos, duration, radius, true))
    {
        delete dynObj;
        return;
    }
...
void Spell::EffectEnchantItemTmp(uint32 i)
{
    ...
    if (m_spellInfo->SpellFamilyName == SPELLFAMILY_SHAMAN && m_spellInfo->SpellFamilyFlags[0] & 0x0000000000400000LL)
    {
        uint32 spell_id = 0;
                ...
                if (m_caster->GetTypeId() == TYPEID_PLAYER)
                    if (Item* item = ((Player*)m_caster)->GetWeaponForAttack(m_attackType, true))
                        if (item->GetProto()->SubClass == ITEM_SUBCLASS_WEAPON_DAGGER)
                            totalDamagePercentMod *= 1.05f;          // 105% to daggers
                        else
                            totalDamagePercentMod *= 0.70f;          // 70% to all other weapons
            }
            ...
            switch(m_spellInfo->Id)
            {
                ...
                case 47193:
                {
                    if (!unitTarget || unitTarget->GetTypeId() != TYPEID_UNIT || !((Creature *)unitTarget)->isPet())
                        return;
                    CreatureInfo const * ci = ObjectMgr::GetCreatureTemplate(unitTarget->GetEntry());
                    switch (ci->family)
                    {
